...
def _see(idx: IdentityIndex, document_id: str, identity: str | None) -> str | None:
    """`peek` immediately followed by `commit` - what `IdentityIndex.see` used
    to do before it was dropped as unused public API (no caller in `src/`;
    `Runner._emit` always needs the peek/commit split, never the fused call -
    see `tests/pipeline/test_runner.py`). Kept here, private to this test
    module, so the four tests below still exercise the fused sequence without
    reintroducing the method itself."""
    first = idx.peek(document_id, identity)
    idx.commit(document_id, identity)
    return first


def test_the_first_sighting_of_an_identity_is_not_a_duplicate() -> None:
    assert _see(IdentityIndex(), "doc-1", "northstar|veritiv|715-33905296") is None


def test_a_second_sighting_names_the_first_document() -> None:
    idx = IdentityIndex()
    _see(idx, "doc-1", "northstar|veritiv|715-33905296")
    assert _see(idx, "doc-2", "northstar|veritiv|715-33905296") == "doc-1"
...
def test_an_unidentifiable_document_is_never_called_a_duplicate() -> None:
    ...
    """
    idx = IdentityIndex()
    assert _see(idx, "doc-1", None) is None
    assert _see(idx, "doc-2", None) is None
...
def test_the_same_document_id_twice_is_a_replay_not_a_duplicate() -> None:
    ...
    idx = IdentityIndex()
    _see(idx, "doc-1", "x")
    assert _see(idx, "doc-1", "x") is None
...
def test_a_peeked_but_never_committed_identity_is_still_unclaimed() -> None:
    ...
    assert _see(idx, "doc-2", "x") is None, "doc-2 must be free to claim it"
