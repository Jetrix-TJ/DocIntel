# Author a persona for `07-centracom` — blind
...
One file: **`07-centracom/persona.json`**, in the format of
...
pages, meta, source = load_document("docs/persona-regeneration/07-centracom/document.pdf")
for p in pages:
    print(p.text)
...
```bash
python3 docs/persona-regeneration/reference/try_persona.py 07-centracom
```
