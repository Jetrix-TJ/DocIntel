# Author a persona for `03-complete-beverage` — blind
...
One file: **`03-complete-beverage/persona.json`**, in the format of
...
```
northstar|complete_beverage_destruction
```
...
pages, meta, source = load_document("docs/persona-regeneration/03-complete-beverage/document.pdf")
for p in pages:
    print(p.text)
...
```bash
python3 docs/persona-regeneration/reference/try_persona.py 03-complete-beverage
```
