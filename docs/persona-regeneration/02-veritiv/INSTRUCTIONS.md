# Author a persona for `02-veritiv` — blind
...
One file: **`02-veritiv/persona.json`**, in the format of
...
pages, meta, source = load_document("docs/persona-regeneration/02-veritiv/document.pdf")
for p in pages:
    print(p.text)
...
```bash
python3 docs/persona-regeneration/reference/try_persona.py 02-veritiv
```
