# Author a persona for `08-comcast` — blind
...
One file: **`08-comcast/persona.json`**, in the format of
...
pages, meta, source = load_document("docs/persona-regeneration/08-comcast/document.pdf")
for p in pages:
    print(p.text)
...
```bash
python3 docs/persona-regeneration/reference/try_persona.py 08-comcast
```
