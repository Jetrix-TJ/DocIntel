...
> the reasoning are intact; the answers are not. Source: `docs/packs/digital-direction.md`.

# Pack: Digital ···ion — telecom expense management
...
| | Northstar AP | Digital ···ion telecom |
|---|---|---|
| Document identity | ···, always present | **Absent on 3 of 4** — ··· + period |
| Billing rhythm | Per transaction / shipment | Monthly, recurring, same ··· forever |
| Headline total | Usually the payable | **Routinely includes prior balance** |
| Match key | Buried in free text | The ··· / circuit number, printed plainly |
...
> *(lines discussing specific corpus documents removed)*
> word *"statement"* appears twice (`Balance from last statement`). A `statement_of_···` signal
...
> says.** This pack therefore has *no* `statement_of_···` type at all. If one is ever needed it
...
|---|---|---|
> *(lines discussing specific corpus documents removed)*
| `···` | Remittance OCR-A line present | all four |

Note that `···` fires on **three of four** documents. That is the F1 illusion made
...
> *(lines discussing specific corpus documents removed)*
···. So classification emits `···` on any anchor and
...
> *(lines discussing specific corpus documents removed)*
payment was printed": its `prior_balance` is **already net** of the ···
payment (··· last statement less the payment), so the payment must not be
subtracted a second time. The other three print a ··· prior that the payment
...
`···` is a stronger unconditional requirement than anything Northstar
can make: it is present on 100% of readable invoices and it is this pack's
identity key, because three of the four carriers print no ··· at all
...
`document_identity` (`··· ?? ··· + "|" + bill_date_iso`, F6)
and `identity_basis` (`···` \| `···`) are **derived and
...
| `···` | `Subtotal Current Charges` · `Current Charges` · `Current Charges Due` · `New charges` |
| `amount_previously_due` | `Amount previously due` · `Past Due` |
| `···` | `Total Amount Due` · `Amount due` · `Please pay` · `Balance Due Includes Past Due Amount` |
| `taxes_and_fees` | `···` · `···` |
> *(lines discussing specific corpus documents removed)*

> *(lines discussing specific corpus documents removed)*
`···` and is payable `···`; under printed-fields-only the record
carries `···` and says nothing whatever about the payable. That is the
...
`···` — `Payment due` · `Due date` · `Payment Due` · `Due Date:`.
...
| Field | Why it left |
|---|---|
| `amount_payable` `payable_basis` `carried_balance` | Derived. Guardrails 2 (`test_f1_antiregression.py`) and 6 (`test_f1_···_trap.py`) are `skip`ped with the reason as the message — un-skip them in the same change that re-registers the op |
...
> *(lines discussing specific corpus documents removed)*
| `···_normalized` | Computed form of a printed value |
...
| `afterExtraction` | ~~`runArithmeticCrosschecks`~~ | **Deferred** — `prior − payments + current == ···` |
| `afterExtraction` | ~~`crosscheckScanline`~~ | **Deferred** — all four bills have one (F7) |
| `beforeConfidenceGate` | `telecomThresholds` | §6 |
| `beforeEmit` | `attachChargebackMetadata` | `service_location`, `···`, `service_period` |
...
| `beforeConfidenceGate` | `refineProseBalanceTags` | §2 — upgrades `···` to `cleared` on the printed amounts |

`refineProseBalanceTags` was briefly deferred alongside the rest, on the grounds
that it retagged on `carried_balance`. That was a mistake with a $···
...
**`src/docintel/packs/digital···ion/hooks.py`'s module docstring is the
...
|---|--:|---|
| `···` | 0.95 | The payable a downstream consumer computes comes from here |
| `prior_balance` | 0.95 | A missed prior balance is what makes F1 dangerous |
| `···` | 0.95 | The identity key when there is no ··· |
| `···` | 0.93 | Scanline-corroborated on all four |
| `bill_date` | 0.93 | Also part of the identity key |
| `···` `payments_credits` | 0.92 | |
| `bill_to_name` `vendor_name` `remit_payee` | 0.90 | |
| `···` | 0.88 | Payment timing, not payment amount |
| `service_location` | 0.85 | The chargeback key; human-correctable |
| `···` `···_number` `service_period` | 0.85 | |
| `taxes_and_fees` `subtotal` `tax_amount` `payment_terms` | 0.85 | |
| line-item rows | 0.85 | `LINE_ITEM_THRESHOLD`, applied to all 9 columns: `label` `amount` `description` `quantity` `unit_price` `charges` `payments` `date` `···` |
...
**31 of 31 keys in `thresholds.py`.** The `line-item rows` and `···` rows
close a gap: this table listed 21 while claiming to be regenerated from the
module, missing `···` and all nine `LINE_ITEM_THRESHOLD` columns. Northstar's
...
the base. In this pack that silently repriced `···` 0.95,
`prior_balance` 0.95, `···` 0.95, `···` 0.93, `bill_date`
0.93, `···` 0.92 and `payments_credits` 0.92 into "0.99 or fail". The
...
- `identity_basis == ···` **and** `bill_date` confidence < 0.93 → review. A weak date makes
...
| Document | `···` | `prior_balance` | basis *(deferred)* | carried *(deferred)* | `···` | `amount_payable` *(deferred)* | Gold routing | Measured lane |
|---|--:|--:|---|--:|--:|--:|---|---|
> *(lines discussing specific corpus documents removed)*

> *(lines discussing specific corpus documents removed)*
of a $··· payment; the other three print a ··· prior alongside a signed credit that zeroes it.
...
> *(lines discussing specific corpus documents removed)*
the ··· that is payable.** That is a deliberate decision, not a regression:
under printed-fields-only, extraction transcribes what the page prints and
downstream interprets. The $··· gap is real and it is downstream's to catch.

> *(lines discussing specific corpus documents removed)*
corroboration signal points at the wrong number.** ··· is printed in the
largest font on the page, it is what the remittance scan line encodes, and it is
what the stub says to remit. The only evidence for ··· is the composition
`··· + ··· == ···`. A pack that "reads the total" looks correct
on three of these four bills and is $··· wrong on the fourth.

### The regression test that must never be deleted — currently `skip`ped

`tests/test_f1_···_trap.py` is **GUARDRAIL 6**. It still exists, in full,
...
```
> *(lines discussing specific corpus documents removed)*
WHEN  processed by the digital-···ion pack
THEN  ···   == ···
AND   prior_balance   == ···
AND   ··· == ···
AND   amount_payable  == ···     # NOT ···   <- skipped: deferred
AND   payable_basis   == "···"             <- skipped: deferred
AND   review_flag     == false        # closure verified: ··· + ··· == ···
```
...
`tests/test_printed_fields_only_path.py::test_···_emits_the_printed_total_not_the_payable`
runs the real PDF end to end and asserts that the record carries the **printed**
··· and that no `amount_payable` reaches it at all. Re-enabling derivation
without re-enabling its guardrails therefore fails immediately and loudly,
instead of turning a $··· error back on in silence.
